...
describe('AppController', () => {
  ...
  it('set materials page', async () => {
    ...
    app.state.hideMattoolPrompt = false;
    app.registerPage(page);
    app.registerController(controller);
    await app.initialize();
    ...
    expect(mockedFn.mock.calls.length).toBe(1);
    expect(app.state.hideMattoolPrompt).toBe(true);
  });
...
describe('updatePageTitle', () => {
  it('should convert istask to false if it is a string', async () => {
    const controller = new AppController();
    const app = new Application();
    const page = new Page({ name: "testpage" });

    app.getLocalizedLabel = jest.fn().mockReturnValue("mocked label");
    ...
    controller.updatePageTitle(parameters);

    expect(parameters.page.params.istask).toBe(false);

    expect(app.getLocalizedLabel).toHaveBeenCalled();
  });

  it('should not change istask if it is already a boolean', async () => {
    const controller = new AppController();
    const app = new Application();
    const page = new Page({ name: "testpage" });
    ...
    expect(parameters.page.params.istask).toBe(true);
...
describe('drillInClassification', () => {
  it('should call commonUtil.drillInClassification with app and item', async () => {
    const controller = new AppController();
    const app = new Application();
    ...
    // Spy on commonUtil.drillInClassification
    const drillInClassificationSpy = sinon.spy(commonUtil, 'drillInClassification');
    const workOrderClassDomain = newDatasource(classificationData, "workOrderClassDomain");
    const testItem = {
      classstructureid: 'TEST123',
      classusewithworkorder: true
    };
    app.registerDatasource(workOrderClassDomain);
    // Call the method
    await controller.drillInClassification(testItem);

    expect(drillInClassificationSpy.calledOnce).toBeTruthy();

    // Restore
    drillInClassificationSpy.restore();
  });
});