...
class MaterialsPageController {
    ...
    this.app = app;
		this.page = page;
		this.page.findDatasource('jmaterialsds').state.loading = true;
		this.page.findDatasource('jtoolsds').state.loading = true;
    }
	...
	pageResumed(page) {
		...
		if (!schedulePage.findDatasource(page.state.selectedDS)) {
		...
		/* istanbul ignore next */
		if (schedulePage.findDatasource(page.state.selectedDS)) {
			schedulePage.findDatasource(page.state.selectedDS).forceReload();
		}
    }
	...
	async loadMaterialsToolsData(evt) {
		let schedulePage = this.app.findPage('schedule') || this.app.findPage("approvals");
		schedulePage.findDatasource(evt.selectedItem.id).forceReload();
	}
	...
	storeSelectedItem(evt) {
		this.page.state[this.page.state.selectedDS+evt.datasource] = this.page.findDatasource(evt.datasource).getSelectedIds();
	}
