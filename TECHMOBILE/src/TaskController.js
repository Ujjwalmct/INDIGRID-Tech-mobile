...
class TaskController {
  ...
  */
  ...
  async _checkGeofence(item) {
    ...
    // Look for geocode on: woDetailResource.item.serviceaddress,
    // then woServiceAddress datasource, then asset service address.
    // If geocode is false → geofence OFF for this WO.
    // ═══════════════════════════════════════════════════════════════
    let geocodeValue = undefined;

    // 2a. From the parent WO item's service address (inline on woDetailResource)
    ...
    // 2b. From the dedicated woServiceAddress datasource (if available)
    ...
    // Fallback to woServiceAddress DS
    ...
    // Fallback to asset service address
    if (isInvalidCoord(lat1, lon1)) {
      const woAssetDs = this.app.findDatasource('woAssetLocationds') || this.page?.findDatasource('woAssetLocationds');
      if (woAssetDs?.item?.serviceaddress) {
        const assetSa = Array.isArray(woAssetDs.item.serviceaddress) ? woAssetDs.item.serviceaddress[0] : woAssetDs.item.serviceaddress;
